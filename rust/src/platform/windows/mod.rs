...
fn worker(hwnd_val: isize, cfg: Config, pending: Arc<Mutex<Option<Vec<Item>>>>) {
    ...
    loop {
        ...
        // Traduction optionnelle des titres absents du cache (1 appel API, langue
        // cible = cfg.language). Les segments fixes ne sont jamais traduits.
        if cfg.translate && !heads.is_empty() {
            if let Ok(key) = std::env::var("GROQ_API_KEY") {
                ...
                todo.sort();
                todo.dedup();
                if let Some(tr) = translate_titles(&todo, &key, &cfg.groq_model, &cfg.language) {
        ...
        // Bandeau = textes fixes (segments, non cliquables) + titres RSS (cliquables).
...
pub fn run(cfg: Config, dev: bool) {
    unsafe {
        SetProcessDpiAwareness(PROCESS_SYSTEM_DPI_AWARE);
        // En mode dev, on ne touche pas au demarrage automatique (laisse la prod).
        if !dev {
            register_autostart();
        }
        ...
        // Bandeau initial : les textes fixes (segments) s'affichent tout de suite ;
        // le worker ajoutera les titres RSS au 1er fetch. Sans segments : placeholder.
        let initial: Vec<Item> = cfg
            .segments
            .iter()
            .map(|s| Item {
                text: s.clone(),
            ...
            .collect();
        let strip = Strip::build(&cfg, dpi, scale, screen_w, &initial);
